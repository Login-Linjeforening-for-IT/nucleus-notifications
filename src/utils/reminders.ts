...
export default async function reminders() {
    ...
    const stored10m = await readFile("10m") as GetEventProps[]
    const stored30m = await readFile("30m") as GetEventProps[]
    const stored1h = await readFile("1h") as GetEventProps[]
    const stored2h = await readFile("2h") as GetEventProps[]
    const stored3h = await readFile("3h") as GetEventProps[]
    const stored6h = await readFile("6h") as GetEventProps[]
    const stored1d = await readFile("1d") as GetEventProps[]
    const stored2d = await readFile("2d") as GetEventProps[]
    const stored1w = await readFile("1w") as GetEventProps[]

    // Filters out events that are ready to be notified about
    const notify10m = stored10m.filter((event: GetEventProps) => timeToEvent(event) <= 600)
    const notify30m = stored30m.filter((event: GetEventProps) => timeToEvent(event) <= 1800)
    const notify1h = stored1h.filter((event: GetEventProps) => timeToEvent(event) <= 3600)
    const notify2h = stored2h.filter((event: GetEventProps) => timeToEvent(event) <= 7200)
    const notify3h = stored3h.filter((event: GetEventProps) => timeToEvent(event) <= 10800)
    const notify6h = stored6h.filter((event: GetEventProps) => timeToEvent(event) <= 21600)
    const notify1d = stored1d.filter((event: GetEventProps) => timeToEvent(event) <= 86400)
    const notify2d = stored2d.filter((event: GetEventProps) => timeToEvent(event) <= 172800)
    const notify1w = stored1w.filter((event: GetEventProps) => timeToEvent(event) <= 604800)

    // Schedules notifications for events 10 minutes away
    notify10m.forEach((event: GetEventProps) => {
    ...
    notify30m.forEach((event: GetEventProps) => {
    ...
    notify1h.forEach((event: GetEventProps) => {
    ...
    notify2h.forEach((event: GetEventProps) => {
    ...
    notify3h.forEach((event: GetEventProps) => {
    ...
    notify6h.forEach((event: GetEventProps) => {
    ...
    notify1d.forEach((event: GetEventProps) => {
    ...
    notify2d.forEach((event: GetEventProps) => {
        const time = `${event.time_start[11]}${event.time_start[12]}:${event.time_start[14]}${event.time_start[15]}`
        const hour = Number(event.time_start[11] + event.time_start[12])
        const ampm = (hour > 0 && hour <= 12) ? "am" : "pm"
    ...
    notify1w.forEach((event: GetEventProps) => {
...
function schedule({ event, counter, textNO, textEN, suffix }: ReminderProps) {
    ...
    // Notification topic
    const norwegianTopic = `n${(event.category.name_no).toLowerCase()}${suffix}`
    const englishTopic = `e${(event.category.name_en).toLowerCase()}${suffix}`
