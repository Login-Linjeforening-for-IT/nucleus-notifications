...
export default function reminders() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let events = yield detailedEvents(true);
        if (!events)
            return handleError({
                file: "reminders",
        ...
        let stored10m = yield readFile("10m");
        let stored30m = yield readFile("30m");
        let stored1h = yield readFile("1h");
        let stored2h = yield readFile("2h");
        let stored3h = yield readFile("3h");
        let stored6h = yield readFile("6h");
        let stored1d = yield readFile("1d");
        let stored2d = yield readFile("2d");
        let stored1w = yield readFile("1w");
        ...
        if (reminders)
            ...
            console.log(`No reminders to be sent at this time ${new Date().toISOString()}`);
    });
}
